...
veryOldInventors = inventors.filter((inventor) => {
    return inventor.year >= 1550 && inventor.year <= 1599
})
...
firstLast = people.map((inventorName) => {
    const [last, first] = inventorName.split(', ')
    return `${first} ${last}`
})
...
travelMethodCounts = travelMethods.reduce((accumulator, index) => {
